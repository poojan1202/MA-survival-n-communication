...
def communicate(obs, actions):

    team1 = []
    team2 = []
    num_agents = len(actions) / 2
    arr_list = []
    for i in range(int(num_agents)):
        arr_list.append(actions[2 * i])
        arr_list[i] = np.array(list(actions[2 * i]) + list(actions[2 * i + 1]))
    actions = tuple(arr_list)


    for i in range(num_agents):
        if obs['agent'][i][1]==0:
            team1.append(obs['agent'][i][0])
        if obs['agent'][i][1]==1:
            team2.append(obs['agent'][i][0])
    ...
    if len(team1)>1:
        ...
        if actions[i1][-2]==0:
            ...
            obs['agent'][i2][-1] = actions[i1][-1]

        if actions[i2][-2]==0:
            obs['agent'][i1][-1] = 0
        else:
            obs['agent'][i1][-1] = actions[i2][-1]

        #print(i1,i2,actions[i1][-2],actions[i1][-1],obs['agent'][i2][-1])
...
class BaseEnv(gym.Env):
    ...
    def step(self, # type: ignore
        ...
        for i in range(int(len(actions)/2)):
            act_list.append([actions[2*i],actions[2*i+1]])

        self.queue_actions(agents, act_list)
        self.simulation.step()
        obs = self.fetch_observations(agents)
        obs = communicate(obs,actions)
...
class MaComms(BaseEnv):
    ...
    def compute_action_space(self):
        ...
        single_action_space = spaces.MultiDiscrete([3,3,3,2,2,2,2])
        cont_action_space = spaces.Box(-1, 1, (1,))
        actions_spaces = (single_action_space,cont_action_space,)*n_agents # type: ignore
    ...
    def queue_actions(self, agents: sim.Group, all_actions: Action):
        ...
        for i, n_a in enumerate(all_actions):
            a,b = n_a
